    ...
    if x < width and y < height:
        ...
        if hit and emission[index]: pixels[y, x] = convert_color_to_rgb(objColor[index]) #collision avec source lumineuse
            ...
            for ray1 in range(sample):  # Sample for reflections (envoie dautre rayon)
                ...
                theta = xoroshiro128p_normal_float32(state, x*ray1) * 2 * math.pi
                phi = max(0,min(math.pi / 2, xoroshiro128p_normal_float32(state, y*ray1) + math.pi / 2))
...
def render_scene():
    ...
    sphere_center = ((0.0, -150.0, 0.0),(0.0, 150.0, 100.0),(-300.0, 0.0, -400.0),(20000.0, 0.0, 0.0))
