...
class Mur:
    ...
    def isInside(self, o, d):
        if d[0]*self.hauteur<=0: return False
        x0 = o[0] + (self.hauteur-o[2])*d[0]/(d[0])
        y0 = o[1] + (self.hauteur-o[2])*d[1]/(d[0])
        z0 = self.hauteur
        if self.normInfiniXZ([x0, y0, z0])>self.longueur: return False
        self.Coord = [x0, y0, z0]
        return True
...
l7it = Mur(0, 400, jaune, [0.999]*3, 0.1, False, 0)
plan = quad(85, 4000, ver, [0.999]*3, 0.1, False, 0)
plan2 = quad(-200, 400, white, [0.4]*3, 0.1, True, 50)
sphere1 = sphere([100, 0, 0], 80, indigo, [0.999]*3, 0.1, False, 0)
...
hierarchy = [sphere1, sphere2, l7it, plan, plan2]
    ...
    for i, object_ in enumerate(hierarchy):
        if object_.isInside(camera_pos, d):
            ...
            for _ in range(sample):
                ...
                new_d = Normalize([loiNormale(i, 10) for i in [0,-1,0]]) 
                sourceRef = [12, -3, 1]
                couleurReflechi = [0]*3
                ...
                couleurObjet = multiVect(couleurObjet, densiteNormale(new_d[0], new_d[1], new_d[2], [0,-1,0]))
