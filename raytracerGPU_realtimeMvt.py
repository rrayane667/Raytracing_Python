...
def rotate(sphere_center, angle, camera):
    for i in range(len(sphere_center)):
        sphere_center[i] = (sphere_center[i][0] - camera[0] ,sphere_center[i][1] - camera[1] ,sphere_center[i][2] - camera[2])
        sphere_center[i]=(sphere_center[i][0], sphere_center[i][1]*np.cos(angle) - sphere_center[i][2]*np.sin(angle), sphere_center[i][1]*np.sin(angle) + sphere_center[i][2]*np.cos(angle))
        sphere_center[i] = (sphere_center[i][0] + camera[0] ,sphere_center[i][1] + camera[1] ,sphere_center[i][2] + camera[2])
    return sphere_center

def mvt(sphere_center, vect, camera):
    for i in range(len(sphere_center)):

        sphere_center[i]=(sphere_center[i][0] + vect[0], sphere_center[i][1] + vect[1], sphere_center[i][2] + vect[2])
...
def render_scene():
    ...
    while running :
        ...
        l = [i for i in sphere_center]
        sphere_center_rot = tuple(rotate(l, mx/100 - width/2, camera_pos))
        
        render_kernel[blockspergrid, threadsperblock](d_pixels, width, height, camera_pos, sphere_center_rot, sphere_radius, num_obj, sample, state, isEmissive, objColor, F0_r, roughness)
